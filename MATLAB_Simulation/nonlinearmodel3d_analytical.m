...
cx = A*rhof*vwindX*abs(vwindX);
cy = A*rhof*vwindY*abs(vwindY);

%{
%Always 0
c1x = 1/sqrt(b*c)*atanh(initVx*sqrt(b/c));
c2x = initX + a/b*log(cosh(c1x*sqrt(b*c)));

c1y = 1/sqrt(b*c)*atanh(initVy*sqrt(b/c));
c2y = initY + a/b*log(cosh(c1y*sqrt(b*c)));
%}

%if case to handle when c2y is negative
if cy > 0
    y_analytical = a/b*(sqrt(b*cy)/a*(t_z0) - 0.693);
else
    y_analytical = -imag(a/b*(sqrt(b*cy)/a*(t_z0) - 0.693));    %Since that part will be from sqrt of the negaive value
end
x_analytical = a/b*(sqrt(b*cx)/a*(t_z0) - 0.693);
