...
for i = 1:2
    ...
    %vwindX = random('Normal', 10,3);
    %vwindY = random('Normal', 10,3);
...
ylabel('z-direction position')
    ...
    % x position
    initX = 0;
diffeqX = @(t,xVel)[(-Cd*A*rhof*xVel^2/(2*mp) +A*rhof*vwindX^2/mp)];
...
% y position
initY = 0;
diffeqY = @(t,yVel)[(-Cd*A*rhof*yVel^2/(2*mp) +A*rhof*vwindY^2/mp)];
